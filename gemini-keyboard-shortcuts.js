...
// @version      1.3.1
// @description  Power-user hotkeys for Gemini with Win11 fixes, URL param prefill, and a centralized config for selectors & shortcuts. Sidebar Toggle = ⌘/Ctrl+B.
// @license      MIT
// @author       Henry Getz
...
(function () {
    ...
    const CFG = {
        ...
        selectors: {
            ...
            themeMenuItemLight: 'button[role="menuitem"] .gds-label-l:contains("Light")', // text-based lookup fallback might be needed
            themeMenuItemDark: 'button[role="menuitem"] .gds-label-l:contains("Dark")',
    ...
    `;
      ...
      `;
        ...
        let lastToggledModel = 3; // Start assuming we might want to go to 1, or default state.
        ...
        function toggleTheme() {
            ...
            waitForElement(CFG.selectors.themeMenuButton, (themeBtn) => {
                ...
                // We utilize a small delay or observer because the submenu animates/renders
                setTimeout(() => {
                    const menuItems = document.querySelectorAll('.mat-mdc-menu-content > button[role="menuitem"]');
                    let lightBtn, darkBtn;

                    // Manual text content search since :contains isn't standard JS
                    menuItems.forEach(btn => {
                        if (btn.textContent.includes('Light')) lightBtn = btn;
                        if (btn.textContent.includes('Dark')) darkBtn = btn;
                    });

                    if (!lightBtn || !darkBtn) {
                        notify('Theme options not found');
                        // Attempt to close menu to clean up
                        document.body.click();
                        return;
                    }

                    const isLightChecked = lightBtn.querySelector(CFG.selectors.themeCheckmark);
                    ...
                    setTimeout(() => {
                        ...
                        const anyMenuOpen = document.querySelector('.mat-mdc-menu-panel.mat-mdc-menu-panel-above'); // standard angular material class for open menus

                        if (mainSettingsBtn) {
                            const isExpanded = mainSettingsBtn.getAttribute('aria-expanded') === 'true';
                            // If button says open, OR we see a menu panel, click the button to toggle it closed
      ...
      `;
            ...
            section('Other', [
                { key: 'Option + Shift + D', action: 'Toggle Theme' },
                { key: '⌘/Ctrl + M', action: 'Cycle Models (1-2-3)' },
